...
const filterSelections = { source: new Set(), department: new Set(), status: new Set() };
...
function filterValues(name) {
  if (name === "source") return [...new Set([...SOURCES, ...actions.map((action) => action.source).filter(Boolean)])];
  if (name === "status") return [...new Set([...STATUSES, ...actions.map((action) => action.status).filter(Boolean)])];
  return [...new Set(actions.map((action) => action.department).filter(Boolean))].sort((a, b) => a.localeCompare(b));
}
function getVisibleActions() {
  ...
  return actions.filter((action) => (!query || searchableFields.some((field) => String(action[field] || "").toLocaleLowerCase().includes(query)))
    && (!filterSelections.source.size || filterSelections.source.has(action.source))
    && (!filterSelections.department.size || filterSelections.department.has(action.department))
    && (!filterSelections.status.size || filterSelections.status.has(action.status))
    && (!elements.overdueFilter.checked || isOverdue(action)));
}
function updateMultiFilter(name) {
  const root = elements[`${name}Filter`], values = filterValues(name), selected = filterSelections[name];
  [...selected].forEach((value) => { if (!values.includes(value)) selected.delete(value); });
  const options = root.querySelector(".multi-filter-options"); options.replaceChildren(...values.map((value) => {
    ...
    checkbox.addEventListener("change", () => { checkbox.checked ? selected.add(value) : selected.delete(value); updateMultiFilter(name); render(); [...options.querySelectorAll("input")].find((item) => item.value === value)?.focus(); });
    const text = document.createElement("span"); text.textContent = value; label.append(checkbox, text); return label;
  }));
  const plural = `${name}${name === "status" ? "es" : "s"}`;
  const summary = !selected.size ? `All ${plural}` : selected.size === 1 ? [...selected][0] : `${selected.size} ${plural} selected`; root.querySelector(".multi-filter-summary").textContent = summary; root.querySelector(".multi-filter-toggle").setAttribute("aria-label", `${name[0].toUpperCase()}${name.slice(1)} filter: ${summary}`);
  const chips = root.querySelector(".filter-chips"); chips.replaceChildren(...[...selected].map((value) => { const chip = document.createElement("button"); chip.type = "button"; chip.className = "filter-chip"; chip.textContent = `${value} ×`; chip.setAttribute("aria-label", `Remove ${value} from ${name} filter`); chip.addEventListener("click", () => { selected.delete(value); updateMultiFilter(name); render(); }); return chip; }));
}
function populateFilters() { ["source", "department", "status"].forEach(updateMultiFilter); }
function renderQualityVisual() {
  const dashboardActions = visibleActions;
  const total = dashboardActions.length;
  const counts = Object.fromEntries(STATUSES.map((status) => [status, dashboardActions.filter((action) => action.status === status).length]));
  const completed = counts.Completed;
  const overdue = dashboardActions.filter(isOverdue).length;
...
function render() {
  ...
  elements.total.textContent = visibleActions.length; elements.open.textContent = visibleActions.filter((action) => action.status !== "Completed").length; elements.overdue.textContent = visibleActions.filter(isOverdue).length; elements.completed.textContent = visibleActions.filter((action) => action.status === "Completed").length; renderQualityVisual();
...
function activeFilterDescription() {
  const parts = [];
  const query = elements.search.value.trim(); if (query) parts.push(`Search: “${query}”`);
  [["Source", "source"], ["Department", "department"], ["Status", "status"]].forEach(([label, name]) => { if (filterSelections[name].size) parts.push(`${label}: ${[...filterSelections[name]].join(" or ")}`); });
  if (elements.overdueFilter.checked) parts.push("Overdue only: Yes");
  return parts.length ? parts.join("; ") : "None (all records)";
}
function exportPdfReport() {
  if (!visibleActions.length) { showMessage("No visible records are available for the PDF report. Change or clear the filters and try again.", true); elements.recordCount.focus?.(); return; }
  try {
    const reportWindow = window.open("", "_blank");
    if (!reportWindow) throw new Error("The report window was blocked. Allow pop-ups for this site and try again."); reportWindow.opener = null;
    const completed = visibleActions.filter((action) => action.status === "Completed").length;
    const overdue = visibleActions.filter(isOverdue).length;
    const rows = visibleActions.map((action) => `<tr><td>${escapeHtml(action.number)}</td><td>${escapeHtml(action.source)}</td><td>${escapeHtml(action.sourceReference || "—")}</td><td class="description">${escapeHtml(action.description)}</td><td>${escapeHtml(action.department)}</td><td>${escapeHtml(action.person)}</td><td>${escapeHtml(formatDate(action.targetDate))}</td><td>${escapeHtml(action.status)}</td><td>${escapeHtml(action.priority)}</td><td>${escapeHtml(formatDate(action.lastUpdate))}</td><td>${evidenceCounts.get(action.id) || 0}</td></tr>`).join("");
    reportWindow.document.write(`<!doctype html><html lang="en"><head><meta charset="utf-8"><title>Tasks / Actions Tracker — Filtered Results Report</title><style>@page{size:A4 landscape;margin:12mm}*{box-sizing:border-box}body{margin:0;color:#243b53;font:10px Arial,sans-serif}header{border-bottom:3px solid #1769aa;padding-bottom:8px}h1{margin:0;color:#102a43;font-size:22px}h2{margin:3px 0 8px;color:#1769aa;font-size:14px}.meta{margin:3px 0;line-height:1.4}.metrics{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin:12px 0}.metric{padding:8px;border:1px solid #cbd8e3;border-radius:4px}.metric strong{display:block;color:#102a43;font-size:17px}table{width:100%;border-collapse:collapse;table-layout:fixed}thead{display:table-header-group}tr{break-inside:avoid;page-break-inside:avoid}th,td{padding:5px;border:1px solid #cbd8e3;text-align:left;vertical-align:top;overflow-wrap:anywhere;white-space:normal}th{background:#e8f2fb;color:#102a43;font-size:8px;text-transform:uppercase}.description{width:20%}footer{margin-top:10px;padding-top:6px;border-top:1px solid #cbd8e3;display:flex;justify-content:space-between;color:#526b80}@media screen{body{padding:18px;max-width:1400px;margin:auto}}</style></head><body><header><h1>Tasks / Actions Tracker</h1><h2>Filtered Results Report</h2><p class="meta"><strong>Generated:</strong> ${escapeHtml(formatDateTime(new Date().toISOString()))}</p><p class="meta"><strong>Applied filters:</strong> ${escapeHtml(activeFilterDescription())}</p><p class="meta"><strong>Exported records:</strong> ${visibleActions.length}</p></header><section class="metrics" aria-label="Report summary"><div class="metric"><strong>${visibleActions.length}</strong>Total</div><div class="metric"><strong>${visibleActions.length - completed}</strong>Open/incomplete</div><div class="metric"><strong>${overdue}</strong>Overdue</div><div class="metric"><strong>${completed}</strong>Completed</div></section><table><thead><tr><th>Action number</th><th>Source</th><th>Source reference number</th><th class="description">Action description</th><th>Responsible department</th><th>Responsible person</th><th>Target date</th><th>Status</th><th>Priority</th><th>Last update</th><th>Evidence count</th></tr></thead><tbody>${rows}</tbody></table><footer><span>Internal use only — handle according to applicable information policies.</span><strong>Powered by Anas Kanakri</strong></footer><script>window.addEventListener("load",()=>{window.focus();window.print()});<\/script></body></html>`);
    reportWindow.document.close(); showMessage(`PDF print view opened for ${visibleActions.length} visible record${visibleActions.length === 1 ? "" : "s"}. Select “Save as PDF” in the browser print dialog.`);
  } catch (error) { showMessage(`PDF report could not be created: ${error.message}`, true); }
}
...
elements.search.addEventListener("input", render); elements.overdueFilter.addEventListener("change", render);
["source", "department", "status"].forEach((name) => { const root = elements[`${name}Filter`], toggle = root.querySelector(".multi-filter-toggle"), popover = root.querySelector(".multi-filter-popover");
  const close = () => { popover.hidden = true; toggle.setAttribute("aria-expanded", "false"); };
  toggle.addEventListener("click", () => { const opening = popover.hidden; document.querySelectorAll(".multi-filter-popover:not([hidden])").forEach((item) => { item.hidden = true; item.previousElementSibling.setAttribute("aria-expanded", "false"); }); popover.hidden = !opening; toggle.setAttribute("aria-expanded", String(opening)); if (opening) popover.querySelector("input")?.focus(); });
  root.querySelector("[data-select-all]").addEventListener("click", () => { filterValues(name).forEach((value) => filterSelections[name].add(value)); updateMultiFilter(name); render(); });
  root.querySelector("[data-clear]").addEventListener("click", () => { filterSelections[name].clear(); updateMultiFilter(name); render(); });
  root.addEventListener("keydown", (event) => { if (event.key === "Escape") { close(); toggle.focus(); } });
});
document.addEventListener("click", (event) => { document.querySelectorAll(".multi-filter").forEach((root) => { if (!root.contains(event.target)) { root.querySelector(".multi-filter-popover").hidden = true; root.querySelector(".multi-filter-toggle").setAttribute("aria-expanded", "false"); } }); });
elements.clear.addEventListener("click", () => { elements.search.value = ""; Object.values(filterSelections).forEach((selection) => selection.clear()); elements.overdueFilter.checked = false; populateFilters(); render(); elements.search.focus(); });
